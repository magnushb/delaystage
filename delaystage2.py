...
##added by Mac on 31.07.2021
class ServerThread(Thread):
    def __init__(self,window): 
        Thread.__init__(self) 
        self.window=window
        self.running=False
        self.host='192.168.0.4' #Server ip
        self.port = 4012
    ...
    def run(self):
        ...
        while self.running:
            ...
            #print(data_split)
            if(data_split[0].lower()=='delay'):
                print('Moving to: '+data_split[1])
                self.window.lineEdit_4.setText(data_split[1])
                self.window.lineEdit_4.setAlignment(QtCore.Qt.AlignCenter)
                self.window.move('abs') 
                print("Movement finished")
                self.s.sendto("Movement finished".encode('utf-8'), addr)
            else:
                self.s.sendto("Invalid command".encode('utf-8'), addr)
    def stop(self):
        self.running=False
        time.sleep(1)
        self.s.close()
